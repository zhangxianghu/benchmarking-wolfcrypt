...
static int _hash_handler(int argc, char **argv) {
    ...
    uint32_t start;
    int ret;
    byte hash_value[WC_SHA256_DIGEST_SIZE];
    wc_Sha256 sha256;
    ...
    ret = wc_Sha256Update(&sha256, input, input_size);  /*can be called again and again*/
    if (ret != 0) {
        return -8;
    }

    ret = wc_Sha256Final(&sha256, hash_value);
    if (ret != 0) {
        return -9;
    }
    ...
    // printf("\n\n");
    // printf("Hash value: ");
    // for (unsigned i = 0; i < WC_SHA256_DIGEST_SIZE; i++)
