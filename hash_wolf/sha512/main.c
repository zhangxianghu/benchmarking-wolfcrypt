...
/* return running time of a function */
...
static int _hash_handler(int argc, char **argv) {
    ...
    uint32_t start;
    int ret;
    byte hash_value[WC_SHA512_DIGEST_SIZE];
    wc_Sha512 sha512;
    ...
    ret = wc_Sha512Update(&sha512, input, input_size);  /*can be called again and again*/
    if (ret != 0) {
        return -8;
    }

    ret = wc_Sha512Final(&sha512, hash_value);
    if (ret != 0) {
        return -9;
    }
    ...
    // printf("\n\n");
    // printf("Hash value: ");
    // for (unsigned i = 0; i < WC_SHA512_DIGEST_SIZE; i++)
