/* Test Camellia encryption and decryption. 
...
static byte* input;
static byte* c_text; 
static byte iv[CAMELLIA_BLOCK_SIZE];
...
static int _encrypt_handler_keyGen(int argc, char **argv) {
    ...
    // puts("Start key generation in 1 second...");
    // xtimer_sleep(1);    
    ...
    // puts("End key generation. Print key in 1 second....\n");
    // xtimer_sleep(1);
...
/* Encrypt a random input with Camellia */
static int _encrypt_handler_camellia(int argc, char **argv) { 
    if (argc < 2) {
    ...
    // printf("Randomly generated input (%d bytes) with padding: ", length);
    ...
    // xtimer_sleep(2);
    // puts("\n-----------------------");
    // puts("\nEncryption starts in 1 second...");
    // xtimer_sleep(1);

    time_start(&start);

    /* set the key */
    ret = wc_CamelliaSetKey(&camEnc, key, CAMELLIA_BLOCK_SIZE, iv);
    if (ret != 0)
        return -3;

    ret = wc_CamelliaCbcEncrypt(&camEnc, c_text, input, length);
    if (ret != 0)
        return -5;
...
static int _decrypt_handler_camellia(int argc, char **argv) {
    ...
    // printf("\n-----------------------");
    // printf("\nStart decryption in 1 seconds...\n");
    // xtimer_sleep(1);

    /* Start decryption */
    time_start(&start);

    ret = wc_CamelliaSetKey(&camDec, key, CAMELLIA_BLOCK_SIZE, iv);
    if (ret != 0)
        return -3;

    ret = wc_CamelliaCbcDecrypt(&camDec, output, c_text, length);
    if (ret < 0)
        return -6;
...
static const shell_command_t shell_commands[] = {
    ...
    { "encrypt", "Encrypt input string", _encrypt_handler_camellia },
    { "decrypt", "Decrypt input string", _decrypt_handler_camellia },
...
int main(void) {
    ...
    puts("=========================");
    puts("Start testing for Camellia");
    puts("=========================\n");

    puts("Key size for Camellia: 128, 192, 256\n\n");
