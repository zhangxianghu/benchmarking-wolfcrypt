...
#define SALT_SIZE 8
#define CCM_NONCE_SIZE 10       // 7 - 13
#define GCM_NONCE_SIZE 12       // 8 - 16, default 12
...
static byte authIn[16];                 // Additional authentication vector (optional)
...
/* Encrypt a random input, support AES-CCM and AES-GCM */

static int _encrypt_handler_ccm(int argc, char **argv) { 
    if (argc < 3) {
    ...
    /* Get encryption mode */
    if(strcmp(argv[1], "ccm") == 0) {
        mode = 1;
        nonce_size = CCM_NONCE_SIZE;
        // printf("nonce size: %d\n", nonce_size);
    ...
    /* Initialize nonce */
    aes_nonce = malloc(nonce_size * (sizeof(byte)));
    ...
    ret = wc_RNG_GenerateBlock(&rng, aes_nonce, nonce_size);
    if (ret != 0)
        return -2;

    // printf("Nonce (%d bytes): ", nonce_size);
    // for(int i = 0; i < nonce_size; i++) {
    //     printf("%X ", aes_nonce[i]);
    // }
    // puts("");

    /* Initialize authentication vector */
    ret = wc_RNG_GenerateBlock(&rng, authIn, 16);
    if (ret != 0)
        return -2;

    // printf("Authentication vector (%d bytes): ", (int)sizeof(authIn));
    // for(int i = 0; i < (int)sizeof(authIn); i++) {
    //     printf("%X ", authIn[i]);
    // }
    // puts("");
    ...
    // xtimer_sleep(3);
    // puts("\n-----------------------");
    // printf("\nEncryption start with mode %d...\n", mode);
    // xtimer_sleep(1);
    time_start(&start);

    if(mode == 1){
        /* sets key */
        ret = wc_AesCcmSetKey(&aes, key, key_size);
        if (ret != 0)
            return -3;

        /* call encryption function */
        ret = wc_AesCcmEncrypt(&aes, c_text, input, input_size, aes_nonce, nonce_size, tag, sizeof(tag), authIn, sizeof(authIn));
    } else {
        /* sets key */
        ret = wc_AesGcmSetKey(&aes, key, key_size);
        if (ret != 0)
            return -3;
        
        /* call encryption function */
        ret = wc_AesGcmEncrypt(&aes, c_text, input, input_size, aes_nonce, nonce_size, tag, sizeof(tag), authIn, sizeof(authIn));
    } 
...
/* Decryption for AES-CCM and AES-GCM */
static int _decrypt_handler_ccm(int argc, char **argv) {
    /* Check argc and print necessary information */
    if (argc < 3) {
        printf("usage: %s [block_mode] [output_size_bytes]\n", argv[0]);
        return 1;
    }
    ...
    // printf("Authentication tag (%d bytes):", sizeof(tag));
    // for(unsigned i = 0; i < sizeof(tag); i++) {
    //     printf("%X ", tag[i]);
    // }
    // puts("");
    ...
    // printf("\n-----------------------");
    // printf("\nStart decryption of mode %d in 2 seconds...\n", mode);
    // xtimer_sleep(2);
    ...
    if(mode == 1){
        /* set keys */
        ret = wc_AesCcmSetKey(&aesdec, key, key_size);
        if (ret != 0) {
            printf("test1, %d\n", ret);
            return -3;
        }
        ...
        ret = wc_AesCcmDecrypt(&aesdec, output, c_text, input_size, aes_nonce, nonce_size, tag, sizeof(tag), authIn, sizeof(authIn));
        if (ret != 0) {
            printf("test2, %d\n", ret);
            return -6;
        }
        ...
        /* set keys */
        ret = wc_AesGcmSetKey(&aesdec, key, key_size);
        if (ret != 0) {
            printf("test1, %d\n", ret);
            return -3;
        }
        ...
        /* call decryption function */
        ret = wc_AesGcmDecrypt(&aesdec, output, c_text, input_size, aes_nonce, nonce_size, tag, sizeof(tag), authIn, sizeof(authIn));
        if (ret != 0) {
            printf("test2, %d\n", ret);
            return -3;
        }
...
static const shell_command_t shell_commands[] = {
    ...
    { "encrypt", "Encrypt input string", _encrypt_handler_ccm },
...
int main(void) {
    ...
    puts("===========================================");
    puts("Start testing for AEAD: AES-CCM and AES-GCM");
    puts("===========================================\n");

    puts("Key size for AES: 128, 192, 256");
    puts("Mode 1 is AES-CCM, mode 2 is AEC-GCM\n\n");
