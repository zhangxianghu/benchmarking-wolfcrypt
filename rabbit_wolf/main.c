/* Test Rabbit encryption and decryption */
...
static byte* key;
static byte* c_text; 
static byte ivRabbit[IV_SIZE];       // IV
...
/* Generate a secret key with specific size: 128 */
static int _encrypt_handler_keyGen(int argc, char **argv) {
    ...
    // puts("Start key generation in one second ...\n");
    // xtimer_sleep(1);    
...
/* Encrypt a random input with rabbit */
static int _encrypt_handler_rabbit(int argc, char **argv) { 
    if (argc < 2) {
    ...
    ret = wc_RNG_GenerateBlock(&rng, ivRabbit, IV_SIZE);
    if (ret != 0)
        return -2;

    // printf("Random IV (%d bytes): ", IV_SIZE);
    // for(unsigned i = 0; i < IV_SIZE; i++) {
    //     printf("%02X ", ivRabbit[i]);
    // }
    // puts("");
    ...
    ret = wc_RabbitSetKey(&rabbitEnc, key, ivRabbit);
    if (ret < 0) {
        printf("Set key error, code: %d\n", ret);
        return -5;
    }

    ret = wc_RabbitProcess(&rabbitEnc, c_text, input, input_size);
    if (ret < 0) {
...
/* Decryption for rabbit */
static int _decrypt_handler_rabbit(int argc, char **argv) {
    ...
    ret = wc_RabbitSetKey(&rabbitDec, key, ivRabbit);
    if (ret < 0) {
        printf("Set key error, code: %d\n", ret);
        return -5;
    }

    ret = wc_RabbitProcess(&rabbitDec, output, c_text, input_size);
    if (ret < 0) {
...
static const shell_command_t shell_commands[] = {
    ...
    { "encrypt", "Encrypt input string", _encrypt_handler_rabbit },
    { "decrypt", "Decrypt input string", _decrypt_handler_rabbit },
...
int main(void) {
    ...
    puts("========================");
    puts("Start testing for Rabbit");
    puts("========================\n");

    puts("Key size for Rabbit: 128 bits");
